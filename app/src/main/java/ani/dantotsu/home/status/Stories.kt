    ...
    private fun loadStory(story: Activity) {
        ...
        when (story.typename){
            "ListActivity" -> {
                ...
                infoText.text = text
                val bannerAnimations = if (PrefManager.getVal(PrefName.BannerAnimations))
                    imageContentViewKen
                else imageContentView
                bannerAnimations.blurImage(story.media?.bannerImage ?: story.media?.coverImage?.extraLarge)
